...
public class WrongController {
  ...
  public String selectall(Model model,QueryVo vo,@RequestParam(required=false,defaultValue="1") Integer page,
		...
		//zuke删除自己的已处理报障记录
			...
			public String insertwrong(Wrong wrong,Model model,HttpSession httpSession){
				User user1= (User) httpSession.getAttribute("user");
				Userlist userlist=userlistService.findhasuserlist(user1.getId());
				wrong.setUserlistId(userlist.getId());
				wrong.setName(userlist.getName());
				solveService.insertwrong(wrong);
				model.addAttribute("error","insertwrong");
				
				return "redirect:mywronglist";
			}
			//管理员查看所有未处理报障
