	...
	<div>
		...
		<div class="search-wrap">
			<div class="search-content">
				<form action="${pageContext.request.contextPath}/wrong/findmysolve" method="post" name="myform">
					<table class="search-tab">
						<tr>
							<th width="70">起始日期:</th>
							<td><input class="common-text" name="fromdate"
								placeholder="报障日期" value="${vo.fromdate}" id="fromdate"
								type="text" readonly></td>
							<th width="70">终止日期:</th>
							<td><input class="common-text" placeholder="报障日期"
								name="todate" value="${vo.todate}" id="todate" type="text"
								readonly></td>
		...
		<div class="result-content">
					...
					<c:forEach items="${solve}" var="solve">
						<tr
							...
							<td><a class="link-update"
								href="${pageContext.request.contextPath}/wrong/deletesolve?id=${solve.id}"
			...
			<div style="LINE-HEIGHT: 20px; HEIGHT: 20px; TEXT-ALIGN: right; margin-top: 10px">
				...
				<c:if test="${ p.pageNum > 1 }">
					[<A href="javascript:to_page(${p.prePage})">前一页</A>]</c:if>
				<input type="hidden" name="page" id="page" value="" /> 第<B>${p.pageNum}</B>页
				<c:if test="${ p.pageNum < p.pages }">
					[<A href="javascript:to_page(${p.nextPage})">后一页</A>]</c:if>
			</div>
	</div>
