...
<body>
	<div>
		...
		<form id="houseForm" name="houseForm"
			action="/applyout/findallapplyout" method=post>
			<div class="result-title">
								...
								<td>${applyout.status}</td>
									...
									<td><c:choose>
											<c:when test="${applyout.status=='申请中'}">
												<a class="link-update"
													href="/applyout/agreeapplyout?houseId=${applyout.houseId }"
													onclick="return window.confirm('确定要同意退租吗？')">同意</a>
											&nbsp;&nbsp; 
											
											<a class="link-del"
													href="/applyout/refuseapplyout?houseId=${applyout.houseId }"
													onclick="return window.confirm('确定要拒绝吗？')">拒绝</a>
											</c:when>
											<c:otherwise>
												<a class="link-del"
													href="/applyout/deleteapplyout?houseId=${applyout.houseId}"
